...
def _suggest_display_name(user_id, device_type):
    """Return a unique friendly device name for the user."""
    base_name = DEVICE_DISPLAY_NAMES.get(device_type or "desktop", DEVICE_DISPLAY_NAMES["desktop"])
    existing_names = {
        row[0]
        for row in db.session.query(UserDevice.display_name)
        .filter(UserDevice.user_id == user_id, UserDevice.display_name.isnot(None))
        .all()
        if row[0]
    }
...
def _is_auto_generated_display_name(name):
    """Check whether a device display name matches the autogenerated pattern."""
    if not name:
        return False
    normalized = name.strip()
    for base_name in DEVICE_DISPLAY_NAMES.values():
        if normalized == base_name or normalized.startswith(f"{base_name} "):
            return True
    return False


def _apply_display_name(device, device_type=None, force=False):
    """Ensure the device has a friendly display name."""
    if device.display_name and not force:
        return
    device.display_name = _suggest_display_name(device.user_id, device_type or device.device_type)
...
def register_device():
    ...
    device = UserDevice(
        ...
        display_name=_suggest_display_name(user.id, "desktop"),
    )
...
def update_device_type(device_id):
    ...
    should_refresh_display_name = not device.display_name or _is_auto_generated_display_name(device.display_name)
    device.device_type = device_type
    device.device_type_confirmed = True
    if should_refresh_display_name:
        _apply_display_name(device, device_type, force=True)
    db.session.commit()
    return jsonify(_serialize_device(device))


@devices_bp.put("/api/devices/<int:device_id>/name")
@handle_route_errors
@require_auth
def update_device_name(device_id):
    """Update the friendly display name for a user's device."""
    user = g.current_user
    payload = request.get_json(silent=True) or {}
    display_name = (payload.get("display_name") or "").strip()

    if not display_name:
        return _bad_request("Missing display_name.")
    if len(display_name) > 128:
        return _bad_request("display_name too long.")
